...
export class ProjectModel {
  ...
  static fromJSON(json: string): ProjectModel {
    ...
    const model = new ProjectModel(data.name, data.author, data.description);
    model.project = {
      ...data,
  ...
  addFloor(name: string, level?: number): Floor {
    const newFloor: Floor = {
      ...
      height: this.project.settings.defaultWallHeight,
      rooms: [],
      walls: [],
      windows: [],
      doors: [],
  ...
  getStatistics(): {
    ...
    return {
      ...
      totalRooms: this.project.floors.reduce((sum, f) => sum + f.rooms.length, 0),
      totalWalls: this.project.floors.reduce((sum, f) => sum + f.walls.length, 0),
      totalFurniture: this.project.floors.reduce((sum, f) => sum + f.furniture.length, 0),
      totalArea: this.project.floors.reduce((sum, f) => 
        sum + f.rooms.reduce((roomSum, r) => roomSum + r.area, 0), 0
      ),
    };
  }
