...
import type { Project, ProjectSettings, Scene, User } from '@types';
...
export class ProjectModel {
  ...
  addCollaborator(userId: string): void {
    if (!this.project.collaborators.includes(userId)) {
      this.project.collaborators.push(userId);
      this.touch();
    }
  }

  removeCollaborator(userId: string): void {
    this.project.collaborators = this.project.collaborators.filter(id => id !== userId);
    this.touch();
  }
  ...
  getStatistics(): {
    ...
    return {
      ...
      totalArea: this.project.scenes.reduce((sum, s) =>
        sum + s.rooms.reduce((roomSum, r) => roomSum + r.area, 0), 0
      ),
    };
  }
