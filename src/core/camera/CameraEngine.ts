...
import type { Vec2, Vec3, CameraState } from '@types';
...
export class CameraEngine {
  ...
  fitToBounds(bounds: { min: Vec3; max: Vec3 }): void {
    ...
    const distance = maxSize / (2 * Math.tan(numeric.degToRad(this.state.fov / 2)));
  ...
  screenToWorld(
    ...
    const tanFov = Math.tan(numeric.degToRad(this.state.fov / 2));
  ...
  private getProjectionMatrix(width: number, height: number): number[] {
    ...
    const fovRad = numeric.degToRad(this.state.fov);
    const f = 1 / Math.tan(fovRad / 2);
    const nf = 1 / (this.state.near - this.state.far);

    return [
      f / aspect, 0, 0, 0,
      0, f, 0, 0,
      0, 0, (this.state.far + this.state.near) * nf, -1,
      0, 0, 2 * this.state.far * this.state.near * nf, 0,
    ];
  }
