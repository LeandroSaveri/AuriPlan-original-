          ...
          <>
            <Section title="Informações" icon={Type}>
              ...
              <div className="grid grid-cols-2 gap-2 mb-3">
                <div>
                  ...
                  <p className="text-sm text-white">{(selectedItem as Room).area.toFixed(2)} m²</p>
                </div>
                <div>
                  <span className="text-xs text-slate-500">Perímetro</span>
                  <p className="text-sm text-white">{(selectedItem as Room).perimeter.toFixed(2)} m</p>
                </div>
              </div>
            </Section>
            <Section title="Cores" icon={Palette}>
              <InputField
                ...
                value={(selectedItem as Room).wallColor}
                onChange={(v) => updateRoom(selectedItem.id, { wallColor: v })}
                type="color"
              />
              <InputField
                label="Cor do Piso"
                value={(selectedItem as Room).floorColor}
                onChange={(v) => updateRoom(selectedItem.id, { floorColor: v })}
                type="color"
              />
              <InputField
                label="Cor do Teto"
                value={(selectedItem as Room).ceilingColor}
        ...
        {isDoor && (
          <>
            ...
            <Section title="Aparência" icon={Palette}>
              <InputField
                label="Cor da Porta"
                value={(selectedItem as Door).panelColor}
                onChange={(v) => updateDoor(selectedItem.id, { panelColor: v })}
                type="color"
              />
              <InputField
                label="Cor da Moldura"
                value={(selectedItem as Door).frameColor}
          ...
          <>
            <Section title="Dimensões" icon={Ruler}>
              ...
              <InputField
                ...
                value={(selectedItem as Window).sillHeight}
            ...
            <Section title="Aparência" icon={Palette}>
              <InputField
                label="Cor da Moldura"
                value={(selectedItem as Window).frameColor}
        ...
        {isFurniture && (
          <>
            <Section title="Transformação" icon={Move}>
              <Vector3Input
                label="Posição"
                values={(selectedItem as Furniture).position}
                onChange={(v) => updateFurniture(selectedItem.id, { position: v })}
              />
              <Vector3Input
                label="Rotação"
                values={(selectedItem as Furniture).rotation.map(r => r * (180 / Math.PI)) as [number, number, number]}
              ...
              <Vector3Input
                label="Escala"
                values={(selectedItem as Furniture).scale}
            ...
            <Section title="Dimensões" icon={Ruler}>
              <div className="grid grid-cols-3 gap-2">
                <div>
                  ...
                  <p className="text-sm text-white">{(selectedItem as Furniture).dimensions.width.toFixed(2)} m</p>
                </div>
                <div>
                  <span className="text-xs text-slate-500">Altura</span>
                  <p className="text-sm text-white">{(selectedItem as Furniture).dimensions.height.toFixed(2)} m</p>
                </div>
                <div>
                  <span className="text-xs text-slate-500">Profundidade</span>
                  <p className="text-sm text-white">{(selectedItem as Furniture).dimensions.depth.toFixed(2)} m</p>
                </div>
              </div>
            </Section>
            <Section title="Aparência" icon={Palette}>
