...
export class PluginManager extends EventEmitter {
  ...
  private createPluginAPI(manifest: PluginManifest): PluginAPI {
    ...
    return {
      ...
      setFloorPlan: createRestrictedAPI('write:floorplan', (fp) => this.api.setFloorPlan(fp)),
      onFloorPlanChange: createRestrictedAPI('read:floorplan', (cb) => this.api.onFloorPlanChange(cb)),
      ...
      onProjectChange: createRestrictedAPI('read:project', (cb) => this.api.onProjectChange(cb)),

      // UI API
      registerToolbarButton: createRestrictedAPI('ui:toolbar', (config) => this.api.registerToolbarButton(config)),
      unregisterToolbarButton: createRestrictedAPI('ui:toolbar', (id) => this.api.unregisterToolbarButton(id)),
      registerSidebarPanel: createRestrictedAPI('ui:sidebar', (config) => this.api.registerSidebarPanel(config)),
      unregisterSidebarPanel: createRestrictedAPI('ui:sidebar', (id) => this.api.unregisterSidebarPanel(id)),
      registerMenuItem: createRestrictedAPI('ui:menu', (config) => this.api.registerMenuItem(config)),
      unregisterMenuItem: createRestrictedAPI('ui:menu', (id) => this.api.unregisterMenuItem(id)),
      ...
      createMaterial: createRestrictedAPI('api:materials', (def) => this.api.createMaterial(def)),

      // Lighting API
      getLightingSystem: createRestrictedAPI('api:lighting', () => this.api.getLightingSystem()),
      createLight: createRestrictedAPI('api:lighting', (def) => this.api.createLight(def)),

      // Camera API
      getCameraSystem: createRestrictedAPI('api:camera', () => this.api.getCameraSystem()),
      setCameraPosition: createRestrictedAPI('api:camera', (x, y, z) => this.api.setCameraPosition(x, y, z)),

      // Storage API
      getStorageItem: createRestrictedAPI('storage:local', (key) => this.api.getStorageItem(`${manifest.id}:${key}`)),
      setStorageItem: createRestrictedAPI('storage:local', (key, value) => this.api.setStorageItem(`${manifest.id}:${key}`, value)),
      removeStorageItem: createRestrictedAPI('storage:local', (key) => this.api.removeStorageItem(`${manifest.id}:${key}`)),
      ...
      fetch: createRestrictedAPI('network:fetch', (url, options) => this.api.fetch(url, options)),
    };
  }
