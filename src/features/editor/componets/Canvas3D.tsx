...
// Door Component
function Door3D({ door, walls }: { door: import('@types').Door; walls: import('@types').Wall[] }) {
  ...
  return (
    <group position={doorPosition} rotation={[0, -angle, 0]}>
      ...
      {/* Frame */}
      <DreiBox args={[door.width + 0.1, door.height + 0.05, door.depth + 0.02]} position={[0, 0, 0]}>
...
// Window Component
function Window3D({ window: win, walls }: { window: import('@types').Window; walls: import('@types').Wall[] }) {
  ...
  windowPosition.y = win.sillHeight + win.height / 2;
...
// Room Component
function Room3D({ room }: { room: import('@types').Room }) {
  ...
  return (
    <group>
      ...
      {/* Ceiling */}
      <mesh geometry={floorGeometry} rotation={[Math.PI / 2, 0, 0]} position={[0, room.height, 0]}>
...
// Furniture Component
function Furniture3D({ item }: { item: import('@types').Furniture }) {
  return (
    <group 
      ...
      scale={item.scale}
    >
      <DreiBox args={[item.dimensions.width, item.dimensions.height, item.dimensions.depth]}>
